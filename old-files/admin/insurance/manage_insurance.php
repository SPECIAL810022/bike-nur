...
if (isset($_GET['id']) && $_GET['id'] > 0) {
	$qry = $conn->query("SELECT * from `insurance` where id = '{$_GET['id']}' ");
	if ($qry->num_rows > 0) {
...
<div class="container-fluid">
	<form action="" id="insurance-form">
		...
		<div class="form-group">
			<label for="provider" class="control-label">Insurance</label>
			<input type="text" name="provider" id="provider" class="form-control form-control-sm rounded-0" value="<?php echo isset($provider) ? $provider : ''; ?>" required />
		</div>
		<div class="form-group">
			...
			$.ajax({
				...
				success: function(resp) {
					if (typeof resp == 'object' && resp.status == 'success') {
						...
						uni_modal('<i class="fa fa-th-list"></i> Insurance Details', 'insurance/view_insurance.php?id=' + resp.cid)
