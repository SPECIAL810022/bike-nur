		...
		<div class="container-fluid">
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<thead>
					<tr>
						<th>#</th>
						<th>Name</th> <!-- Title for the column -->
						<th>Father's Name</th> <!-- Title for the column -->
						<th>Contact Number</th> <!-- Title for the column -->
						<th>Action</th>
					</tr>
				</thead>
				<tbody>
					<?php
					$i = 1;
					$qry = $conn->query("SELECT t.*, t.id as transtion_id , customers.*
            FROM transaction_list t
            LEFT JOIN vehicle_list v ON t.vehicle_id = v.id
            LEFT JOIN model_list m ON v.model_id = m.id
            LEFT JOIN brand_list b ON m.brand_id = b.id
					...
					?>
						<tr>
							...
							<td align="center" class='text-center'>
								<a class="btn btn-flat btn-light bg-gradient-light btn-sm border" href="./?page=vehicles/view_transaction&id=<?= $row['transtion_id'] ?>"><i class="fa fa-eye"></i> View</a>
							</td>
						</tr>
					<?php
					endwhile;
