...
class Master extends DBConnection
{
	...
	{
		extract($_POST);
		$get = $this->conn->query("SELECT LeadID FROM `leads` WHERE LeadID = '{$id}'");
		$del = $this->conn->query("DELETE FROM `leads` WHERE LeadID = '{$id}'");
		if ($del) {
			$resp['status'] = 'success';
	...
	function save_next_followup()
	{
		...
		if (empty($leadId)) {
			$resp = array('status' => 'failed', 'msg' => 'Lead ID is missing.');
			return json_encode($resp);
		}
		...
		if ($save_followup) {
			...
			if ($update_lead) {
				$resp = array('status' => 'success', 'msg' => 'Next follow-up details successfully saved.');
			} else {
				$resp = array('status' => 'failed', 'msg' => 'Failed to update LeadStatus. ' . $this->conn->error);
			}
		} else {
			$resp = array('status' => 'failed', 'msg' => 'Failed to save next follow-up details. ' . $this->conn->error);
		}
	...
	function save_booking()
	{
		...
		$contactNumber = $this->conn->real_escape_string($contactNumber);
		$fathersName = $this->conn->real_escape_string($FathersName);
		$address = $this->conn->real_escape_string($Address);
		$pincode = $this->conn->real_escape_string($Pincode);
		$age = $this->conn->real_escape_string($Age);
		$bookingDate = $this->conn->real_escape_string($BookingDate);
		$estimateDeliveryDate = $this->conn->real_escape_string($EstimateDeliveryDate);
		$scheme = $this->conn->real_escape_string($Scheme);
		...
		if (isset($_FILES['DocumentUpload']) && $_FILES['DocumentUpload']['error'] == 0) {
			...
			$documentName = basename($_FILES['DocumentUpload']['name']);
		}

		// Prepare SQL query to insert into bookings table
		$sql_booking = "INSERT INTO `bookings` 
			(`LeadID`, `Name`, `ContactNumber`, `FathersName`, `Address`, `Pincode`, `Age`, `BookingDate`, `EstimateDeliveryDate`, `Scheme`, `Amount`, `Notes`, `PaymentMethod`, `RefNumber`, `BookingType`, `BikeDetails`, `GiftItems`, `DocumentType`, `DocumentUpload`) 
			VALUES 
			('{$leadId}', '{$name}', '{$contactNumber}', '{$fathersName}', '{$address}', '{$pincode}', '{$age}', '{$bookingDate}', '{$estimateDeliveryDate}', '{$scheme}', '{$amount}', '{$notes}', '{$paymentMethodInput}', '{$refNumberInput}', '{$bookingType}', '{$bikeDetails}', '{$giftItems}', '{$DocumentType}', '{$documentName}')";

		$save_booking = $this->conn->query($sql_booking);
		if ($save_booking) {
			// Insert into customers table
			if ($DocumentType == 'AADHAAR') {
				$documentColumn = 'aadhaar_card';
			} elseif ($DocumentType == 'PAN') {
				$documentColumn = 'pan_card';
			} elseif ($DocumentType == 'VOTER') {
				$documentColumn = 'voter_file';
			} else {
				// Handle unexpected document type if needed
			}

			// Construct the SQL query based on the selected document type
			$sql_customer = "INSERT INTO `customers` (`name`, `father_name`, `address`, `pin`, `contact_number`, `age`, `created_at`, `user_name`, `$documentColumn`) 
				VALUES ('$name', '$fathersName', '$address', '$pincode', '$contactNumber', '$age', NOW(), '{$_SESSION['userdata']['id']}', '$documentName')";

			// Attempt to save customer details
			$save_customer = $this->conn->query($sql_customer);
			if ($save_customer) {
				$sql_update_lead = "UPDATE `leads` SET `LeadStatus`='BOOKING DONE' WHERE `LeadID`='{$leadId}'";
				$update_lead = $this->conn->query($sql_update_lead);
				$resp['status'] = 'success';
				$resp['msg'] = 'Booking and customer details successfully saved.';
			} else {
				// Error handling if inserting into customers table failed
				$resp['status'] = 'failed';
				$resp['err'] = $this->conn->error . " [{$sql_customer}]";
			}
		} else {
	...
	function delete_booking()
	{
		...
		// Get the leadId associated with the booking
		$get_lead_id = $this->conn->query("SELECT LeadID FROM `bookings` WHERE BookingID = '{$id}'");
		if ($get_lead_id->num_rows > 0) {
			$row = $get_lead_id->fetch_assoc();
			$leadId = $row['LeadID'];
		} else {
			// Return an error response if the booking doesn't exist or the leadId is not found
			$resp['status'] = 'failed';
			$resp['error'] = 'Lead ID not found for the given Booking ID.';
			return json_encode($resp);
		}

		// Delete the booking
		$del = $this->conn->query("DELETE FROM `bookings` WHERE BookingID = '{$id}'");
		if ($del) {
			$resp['status'] = 'success';
			$resp['msg'] = "Booking has been deleted successfully.";
			// Additional actions after deleting booking, if needed

			// Update lead status if necessary
			$sql_update_lead = "UPDATE `leads` SET `LeadStatus`='BOOKING CANCELED' WHERE `LeadID`='{$leadId}'";
			$update_lead = $this->conn->query($sql_update_lead);
		} else {
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
	...
	{
		// Retrieve lead ID and user ID from POST request
		$leadId = $_POST['leadId'];
		$userId = $_POST['userId'];

		// Perform update query
		$sql = "UPDATE leads SET user_name = '$userId' WHERE LeadID = $leadId";
		if ($this->conn->query($sql) === TRUE) {
			$resp['status'] = 'success';
			$resp['msg'] = 'Lead assigned successfully.';
		} else {
			$resp['status'] = 'failed';
